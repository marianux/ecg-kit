...
while( loop_count < MAX_LOOPS && crit(max(loop_count-1,1)) > 0.001 )
    ...
    realizations = cellfun(@(a)( cell2mat(arrayfun( @(aa)( conv(double(signal(a,aa)) - mean(double(signal(a,aa))), esemble_avg(:,aa) ) ), 1:nsig, 'UniformOutput', false )) ), aux_idx, 'UniformOutput', false);
