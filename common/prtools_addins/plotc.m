...
% 	end
	...
	if isaffine(w) & c == 2 & ~strcmp(s,'col')	% plot as vector
        ...
        bMyDomainAux = iscell(cDomainAux);
        
        if( bMyDomainAux && ~isempty(cell2mat(cDomainAux)) )
        ...
        if( bMyDomainAux && ~isempty(cDomainAux) )
		...
		if ~strcmp(s,'col')
			...
			for j=1:c
				...
				col = col + 1;
                cc = contourc([V(1)-dx:dx:V(2)+dx],[V(3)-dy:dy:V(4)+dy],Z,[0 0]);
				if ~isempty(cc)
					[~, h] = contour([V(1)-dx:dx:V(2)+dx],[V(3)-dy:dy:V(4)+dy],Z,[0 0]);
					hh = [hh;h];
					while ~isempty(cc)
						len = cc(2,1);
						h = fill(cc(1,2:len+1),cc(2,2:len+1),map(col,:),'FaceAlpha',0.5);
                        hh = [hh;h];
						cc(:,1:len+1) = [];
					end
				end
