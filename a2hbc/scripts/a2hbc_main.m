...
while ( ~bUserExit && repeat_idx <= Repetitions )
    ...
    try
        ...
        if( bRecordingChanged )
            ...
            warning('TODO: generalizar para featmatrices arb grandes.');
            ...
            if( isempty( CachedFeatMatFiles ) )
                ...
                ECGtask_class_fc_hdl.recording_format = ECGw.recording_format;
                ECGtask_class_fc_hdl.autovec = ECGtask_PCA_proj_basis_hdl.autovec;
                ECGtask_class_fc_hdl.user_string = 'tmpfile_a2hbc_';
            ...
            if( isempty( CachedFeatMatFiles ) )
                error( 'a2hbc:FeaturesNotCalc', 'Features files not found. Check temporary dir cached fil.\n' );
            else
